 ...
 * @description Interactive particle system with noise displacement
 * @performance InstancedMesh for 10k+ particles, GPU-based animation
 */
...
const ParticleSystem: React.FC<ParticleSystemProps> = ({
  ...
  const vertexShader = `
    ...
    void main() {
      ...
      float noise = snoise(vec3(pos * 0.5 + uTime * 0.2));
      ...
      if (dist < 3.0) {
        pos += normalize(toMouse) * (3.0 - dist) * 0.2;
      }
...
export const ParticleField: React.FC<ParticleFieldProps> = ({ className }) => {
  return (
    <div className={`absolute inset-0 ${className}`}>
      ...
      >
        <ParticleSystem mouseX={0} mouseY={0} count={3000} />
      </Canvas>
    </div>
  );
};
